...
$$
\begin{align}
\left\lvert\sum_{k=N+1}^\infty a_k \right\rvert < \epsilon
\end{align}
$$
...
2. $p\in E^\prime$
  ...
  $$
  \begin{align}
  0<\delta < \min\{\lvert p-x_k\rvert: k=1,2,\ldots, N\} 
  \end{align}
  $$
...
2. $x_n \in E^\prime$.   
  ...
  $$
  \begin{align}
   0<\delta <\min\{\lvert x_n-x_k\rvert :k=1,\ldots,N, k\neq n\}
     \end{align}
  $$
  
  Then, $x_k \not\in (x_n, x_n+\delta)$ for $k=1,\ldots,N$. Thus, for $y\in (x_n, x_n+\delta), I(y-x_k)=I(x_n-x_k)$ for all $k=1,\ldots,N$. 

 For $y>x_n, 0\leq I(y-x_k) - I(x_n-x_k)\leq 1 \text{ for all } k\in\mathbb{N}$
...
$$
\begin{align}
0<\delta <\min\{\lvert x_k-x_n\rvert :k=1,\ldots,N, k\neq n\}
\end{align}
$$
...
&\iff \lvert f(y) - (f(x_n)-c_n)\rvert < \epsilon
\end{split}
\end{align}
$$
...
<*proof*>
Without loss of generality, assume that $f$ is strictly increasing function. Let $x,y\in I$ with $x<y$ be given. Since $f$ is continuous, by corollary 4.2.12, $f(I) = J$ is an interval.  
...
2. $x_0-\epsilon \in I$
  Choose a $\delta = y_0 - f(x_0-\epsilon)=f(x_0) = f(x_0-\epsilon)>0$. 
