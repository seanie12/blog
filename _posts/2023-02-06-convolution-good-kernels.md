...
Note that $g$ is continuous on the compact set $[-\pi, \pi]$, $g$ is uniformly continuous on $[-\pi, \pi]$. Thus, there exists  $\delta>0$ such that  $\lvert p -q \rvert <\delta \Rightarrow\lvert g(p) - g(q)\rvert <\epsilon/ B$, where $B$ is a bound on $f$, i.e., $\lvert g(x)\rvert \leq B$ for all $x\in[-\pi, \pi]$. Now consider,
$$
\begin{align*}
