...
Suppose that  $\sup A = r_x \neq r_y.$ Then for any $r \in (r_x, r_y), \sup A \geq r$ because $r \in (r_x, r_y) \subset [r_x, r_y)\subsetA$. But it implies that $\sup A \geq r > r_x$, which contradicts to the assumption that $\sup A = r_x$. 
