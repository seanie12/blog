...
For any $k\in (l_x, l_y], k\in U.$ i.e. $k\in D.$ If $l_y = l_x$, it is trivial case. Even if $l_x\neq l_y, however, we can show that l_y = \inf D = l_x.$ Suppose that $l_y = \inf D \neq l_x$. Then for any $q \in (l_x, l_y), \inf D \leq q < l_y$, which contradicts to the assumption that $l_y = \inf D$. Thus, $l_x = l_y$.
