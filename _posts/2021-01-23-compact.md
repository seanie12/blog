...
Then $N^\prime_\epsilon(p) \cap S=\empty.$ Thus $p$ is not a limit point of $E$. 
...
Suppose that $K$ is not closed. Then there is $x_0\in K^c$ such that $x_0$ is a limit point of $K$. Then, for every $n\in\mathbb{N}$, there is $a_n\in N_{1/n}(x_0)\cap K$. We want to say that $x_0$ is the unique limit point of $$\{a_n\}$$.
...
$$
since $x_0 \notin  E$ and $a_n\in E$.
In other words, $$N^\prime_\epsilon(x_0)\cap \{a_n\}=\empty.$$ Now let $y\in K$ be given. With triangular inequality,
