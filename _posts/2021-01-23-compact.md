...
Then  $N_\epsilon (p) \cap N_{\epsilon}(q_i) = \emptyset$ for all $i=1,\ldots, n$. Since $\bigcup_{i=1}^nN_{\epsilon_{q_i}}(q_i)$ is an open cover of $K, N_\epsilon (p) \subset K^c,$ i.e. $p \in \text{Int}(K^c)$. <br />
