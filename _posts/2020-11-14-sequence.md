...
$$
Thus, $|\frac{1}{n^p}| < \frac{1}{N^p} <\epsilon$.
...
&> \frac{n^k}{2^kk!} q^k \:(\becuase n-k+1 > \frac{n}{2}+1 > \frac{n}{2})\\ 
\end{split}
\end{align}
$$
Thus, $0< \frac{n^\alpha}{p^n} <\frac{2^k k!}{q^kn^{k-\alpha}}$. Since $k-\alpha>0, \displaystyle{\lim_{n\to\infty}1/n^{k-\alpha}}=0.$

$\therefore \displaystyle{\lim_{n\to\infty}\frac{n^\alpha}{p^n}} =0$
$$\tag*{$\square$}$$

(e) Let $p=1\pm \frac{1}{q}$ for some $q>1$.
$$
\begin{align}
|p^n| = |p|^n = \frac{1}{q^n}
\end{align}
...
$$
|\frac{p^n}{n!}| = \frac{|p|^n}{n!} = \frac{|p|^n}{(k-1)!k\cdots (n-1)n}\\
< \frac{|p|^n}{(k-1)!k^{n-k+1}}\\
=\frac{k^{k-1}}{(k-1)!}\frac{|p|^n}{k^n}
$$ 
