...
$$
\begin{align*}
N_\epsilon (p) &\coloneqq\{ x\in \mathbb{R}: |x-p|<\epsilon \} \\
...
$$
\begin{align*}
M:= \max\{ |p_1|, \ldots, |p_N|, |L|+1\} > 0.
\end{align*}
$$ 

Then $|p_n| \leq M$ for all $n\in M$.
...
(b) Let $\epsilon >0$ be given. Since $(a_n)_{n=1}^\infty$ converges, $(a_n)_{n=1}^\infty$ is a bounded sequence so that we can find $M>0$ s.t. $|a_n|\leq M$ for all $n\in \mathbb{N}$.  

By our assumption, there exists $N\in\mathbb{N}$ s.t. $n\geq N\Rightarrow |a_n-a|<\frac{\epsilon}{|b|}$ and $|b_n-b| < \frac{\epsilon}{2M}$. If $n\geq N$, then
...
\end{align*}
$$
Since $|a|-|a_n| \leq |a_n-a|, |a_n|>\frac{|a|}{2}$ for $n\geq N_2$. Put 
...
(d) $p>1, \alpha \in\mathbb{R}\Rightarrow \displaystyle{\lim_{n\to\infty}\frac{n^\alpha}{p^n}=0}$ 
 ...
 (e) $|p|<1 \Rightarrow \displaystyle{\lim_{n\to\infty}}p^n =0$ 
