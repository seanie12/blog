...
(b) Let $\epsilon >0$ be given. Since $(a_n)_{n=1}^\infty$ converges, $(a_n)_{n=1}^\infty$ is a bounded sequence so that we can find $M>0$ s.t. $|a_n|\leq M$ for all $n\in \mathbb{N}$. <br /> By our assumption, there exists $N\in\mathbb{N}$ s.t. $n\geq N\Rightarrow |a_n-a|<\frac{\epsilon}{|b|}$ and $|b_n-b| < \frac{\epsilon}{2M}$. If $n\geq N$, then
